...
, m_avFrame(NULL)
{
	...
	m_screen = SDL_SetVideoMode(m_avCodecContext->width, m_avCodecContext->height, 0, 0);
	if (!m_screen) {
		throw std::runtime_error("SDL: could not set video mode - exiting");
	}

	// Allocate a place to put our YUV image on that screen
	m_bmp = SDL_CreateYUVOverlay(m_avCodecContext->width, m_avCodecContext->height, SDL_YV12_OVERLAY, m_screen);
...
void StreamMediaSink::afterGettingFrame(unsigned frameSize, unsigned numTruncatedBytes, struct timeval presentationTime, unsigned /*durationInMicroseconds*/)
{
	...
	while (packet.size > 0) {
		len = avcodec_decode_video2(m_avCodecContext, m_avFrame, &gotFrame, &packet);
		if (len < 0) {
			break;
		}
		if (gotFrame) {
